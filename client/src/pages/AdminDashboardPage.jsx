...
export default function AdminDashboardPage() {
  ...
  const [witanimeQuery, setWitanimeQuery] = useState("");
  const [witanimeResults, setWitanimeResults] = useState([]);
  const [witanimeBusy, setWitanimeBusy] = useState(false);
  ...
  const importMal = async () => {
    try {
      const { data } = await api.post("/admin/anime/import-jikan", {
        malId: Number(malId),
      });
  ...
  const searchConsumet = async () => {
    ...
    const loadingToast = toast.loading(
      `جارٍ البحث في ${consumetProvider} عن "${consumetQuery.trim()}"…`,
    );
    try {
      ...
      if (results.length === 0) {
        toast.error("لم يتم العثور على نتائج — جرب كتابة مختلفة.", {
          id: loadingToast,
        });
      } else {
  ...
  const importConsumetEpisodes = async (consumetAnimeId) => {
    ...
    try {
      const { data } = await api.post(
        `/admin/anime/${selectedAnime}/episodes/import-consumet`,
        {
      ...
      toast.success(
        `تم استيراد ${data.created} حلقة (تم تخطي ${data.skipped?.length || 0} مكررة)`,
      );
      await loadEpisodes(selectedAnime);
      await loadAnime();
    } catch (e) {
  ...
  const searchWitanime = async () => {
    if (!witanimeQuery.trim()) return;
    setWitanimeBusy(true);
    setWitanimeResults([]);
    const loadingToast = toast.loading(
      `جارٍ البحث في ويتانيمي عن "${witanimeQuery.trim()}"…`,
    );
    try {
      const { data } = await api.get("/admin/witanime/search", {
        params: { q: witanimeQuery.trim() },
        timeout: 30000,
      });
      const results = data.data || [];
      setWitanimeResults(results);
      if (results.length === 0) {
        toast.error("لم يتم العثور على نتائج — جرب كتابة مختلفة.", {
          id: loadingToast,
        });
      ...
      setWitanimeBusy(false);
    }
  };

  const importWitanimeEpisodes = async (witanimeAnimeId) => {
    if (!selectedAnime) {
      toast.error("اختر أنمي من الكتالوج أولاً");
      return;
    }
    ...
    try {
      const { data } = await api.post(
        `/admin/anime/${selectedAnime}/episodes/import-witanime`,
        {
          witanimeAnimeId,
          season: episodeForm.season,
        },
      ...
      <div className="mx-auto max-w-6xl space-y-6 px-4 py-8 lg:px-8">
        <div>
          ...
          <p className="text-sm text-zinc-500">
            إدارة الكتالوج، البث، الصفحة الرئيسية، والمستخدمين.
          </p>
        </div>
        ...
        {tab === "import" && (
          <div className="glass space-y-4 rounded-2xl p-6">
            <h2 className="text-lg font-bold text-white">
              استيراد من MyAnimeList (Jikan)
            </h2>
            <p className="text-sm text-zinc-500">
              أدخل رقم MAL (مثال: 16498 = Attack on Titan).
            </p>
        ...
        {tab === "episodes" && (
          <motion.div className="space-y-6">
            ...
            <div className="glass space-y-4 rounded-2xl p-6">
              <h2 className="text-lg font-bold text-white">
                استيراد حلقات عبر Consumet
              </h2>
              <p className="text-sm text-zinc-500">
                استيراد الحلقات من مزودي Consumet. يتم حل روابط البث عند
                المشاهدة.
              </p>
              <motion.div className="flex flex-wrap gap-3">
                ...
                <input
                  ...
                  onKeyDown={(e) =>
                    e.key === "Enter" && (e.preventDefault(), searchConsumet())
                  }
            ...
            <div className="glass space-y-4 rounded-2xl p-6">
              <h2 className="text-lg font-bold text-white">
                استيراد حلقات من Witanime
              </h2>
              <p className="text-sm text-zinc-500">
                استيراد الحلقات من ويتأنيمي (العربية). يتم حل روابط البث عند
                المشاهدة.
              </p>
              <motion.div className="flex flex-wrap gap-3">
                <input
                  value={witanimeQuery}
                  onChange={(e) => setWitanimeQuery(e.target.value)}
                  onKeyDown={(e) =>
                    e.key === "Enter" && (e.preventDefault(), searchWitanime())
                  }
                  placeholder="ابحث في ويتأنيمي…"
                  className="min-w-[200px] flex-1 rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-sm text-white"
                />
                <button
                  type="button"
                  disabled={witanimeBusy}
                  onClick={searchWitanime}
                  className="rounded-xl bg-zinc-800 px-4 py-2 text-sm font-semibold text-white hover:bg-zinc-700 disabled:opacity-50"
                >
                  {witanimeBusy ? "جارٍ البحث…" : "بحث"}
                </button>
              </motion.div>
              {witanimeResults.length > 0 && (
                <ul className="max-h-64 space-y-2 overflow-y-auto text-sm">
                  {witanimeResults.map((r) => (
                    <li
                      key={r.id}
                      className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-white/10 bg-black/30 p-3"
                    >
                      <div className="flex min-w-0 items-center gap-3">
                        ...
                        <div>
                          <p className="font-semibold text-white">{r.title}</p>
                          <p className="text-xs text-zinc-500">Witanime</p>
                        </div>
                      </div>
                      <button
                        type="button"
                        disabled={witanimeBusy || !selectedAnime}
                        onClick={() => importWitanimeEpisodes(r.id)}
                        className="rounded-lg bg-orange-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-orange-500 disabled:opacity-40"
                      >
                        استيراد الحلقات
                      </button>
                    </li>
                    ...
                    {episodes.map((ep) => (
                      ...
                      >
                        <span className="text-zinc-300">
                          ...
                          {ep.streamSource === "witanime" && (
                            <span className="mr-2 rtl:ml-2 rounded bg-orange-900/40 px-1.5 py-0.5 text-[10px] uppercase text-orange-200">
                              witanime
                            </span>
                          )}{" "}
                        </span>
            ...
            >
              ...
              {previewUrl && (
                <div className="space-y-2 rounded-xl bg-black/50 p-3 border border-white/5">
                  <div className="flex items-center justify-between text-xs text-zinc-400">
                    <span>
                      معاينة المشغل (
                      {previewType === "auto" ? "كشف تلقائي" : previewType})
                    </span>
                    <button
                      type="button"
              ...
              <button
                type="submit"
                className="w-full rounded-xl bg-red-600 py-3 text-sm font-bold text-white shadow-lg transition-transform hover:scale-[1.01] active:scale-[0.99] hover:bg-red-500"
              >
                حفظ الحلقة
              </button>
            </form>
              ...
              {banners.map((b) => (
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() =>
                      api.delete(`/admin/banners/${b._id}`).then(loadBanners)
                    }
                  >
                    إزالة
                  </button>
        ...
        {tab === "sections" && (
          <div className="glass space-y-4 rounded-2xl p-6">
            ...
            <p className="text-sm text-zinc-500">
              الأقسام تسحب البيانات مباشرة من التصنيفات والتقييمات.
            </p>
            <form
              className="space-y-3"
              onSubmit={async (e) => {
            ...
            >
              ...
              <input
                name="genre"
                className="w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-sm text-white"
                placeholder="التصنيف (لنوع التصنيف)"
              />
              <input
                name="order"
                type="number"
              ...
              {sections.map((s) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="text-red-400"
                    onClick={() =>
                      api.delete(`/admin/sections/${s._id}`).then(loadSections)
                    }
                  >
                    إزالة
                  </button>
