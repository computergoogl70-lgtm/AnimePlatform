  ...
  return (
    <AppShell genre={genre} onGenre={onGenre}>
      <div className="relative min-h-[70vh] overflow-hidden">
        ...
        <div className="relative mx-auto flex max-w-7xl flex-col gap-6 px-4 pb-16 pt-24 lg:flex-row lg:items-end lg:px-8 lg:pb-24 lg:pt-32">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="max-w-2xl space-y-4">
            <p className="text-sm font-semibold uppercase tracking-widest text-red-400">Featured</p>
            <h1 className="text-4xl font-black leading-tight text-white drop-shadow-lg md:text-6xl">
              {hero?.title || 'Stream the worlds you love'}
            </h1>
            <p className="max-w-xl text-sm text-zinc-300 md:text-base">
              {hero?.description?.slice(0, 220) || 'Dark, cinematic UI with smooth motion—built for modern anime catalogs and external HLS streams.'}
            ...
            {hero && (
              <div className="flex flex-wrap gap-3 text-xs text-zinc-300 md:text-sm">
                ...
                <span>{hero.year || '—'}</span>
                <span>{hero.episodeCount ? `${hero.episodeCount} Episodes` : ''}</span>
          ...
          <div className="glass mx-auto mt-8 w-full max-w-md rounded-2xl p-4 lg:mt-0">
            <p className="text-xs font-semibold uppercase tracking-wide text-zinc-400">Quick search</p>
            ...
            <p className="mt-2 text-xs text-zinc-500">Press Enter to search the catalog and live metadata hints.</p>
          </div>
        </div>
      </div>
      ...
      {!loading && home?.continueWatching?.length > 0 && (
        <SectionRow title="Continue watching" items={home.continueWatching.map((w) => w.animeId).filter(Boolean)} />
      )}
      ...
      {!loading && !home?.sections?.length && (
        <>
          <SectionRow title="Trending now" items={home?.trending} />
          <SectionRow title="Top rated" items={home?.topRated} />
          <SectionRow title="Recently added" items={home?.recent} />
        </>
      )}
      ...
      {!loading && home?.popularGenres?.length > 0 && (
        <section className="py-10">
          <div className="mx-auto max-w-7xl px-4 lg:px-8">
            <h2 className="mb-4 text-xl font-bold text-white md:text-2xl">Popular genres</h2>
            <div className="flex flex-wrap gap-2">
              {home.popularGenres.map((g) => (
                ...
                >
                  {g.name}
                  <span className="ml-2 text-xs text-zinc-500">{g.count}</span>
                </button>
              ))}
            </div>
          </div>
