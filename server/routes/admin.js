...
router.post('/anime/import-jikan', async (req, res) => {
  ...
  const existing = await Anime.findOne({ $or: [{ malId: doc.malId }, { slug: doc.slug }] });
  if (existing) {
    return res.status(409).json({ message: 'Anime already exists', anime: existing });
  }
...
router.get('/consumet/info', async (req, res) => {
  ...
  if (!provider || !animeId) {
    return res.status(400).json({ message: 'provider and animeId are required' });
  }
  try {
...
router.post('/anime/:id/episodes/import-consumet', async (req, res) => {
  ...
  if (!provider || !consumetAnimeId) {
    return res.status(400).json({ message: 'provider and consumetAnimeId are required' });
  }
  try {
  ...
  try {
    ...
    if (!anime.description && info.description) anime.description = info.description;
...
router.put('/sections/:id', async (req, res) => {
  const s = await HomeSection.findByIdAndUpdate(req.params.id, req.body, { new: true });
