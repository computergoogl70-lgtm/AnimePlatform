...
import { witanimeEpisodeStream } from "../services/witanime.js";
...
router.get("/:episodeId", requireAuth, async (req, res) => {
  try {
    ...
    const anime = await Anime.findById(ep.animeId)
...
function detectSourceType(url) {
  ...
  if (
    ...
    cleanUrl.includes("application/x-mpegurl")
  ) {
    return "m3u8";
  }
  if (
...
router.get("/:episodeId/stream", requireAuth, async (req, res) => {
  try {
    ...
    if (ep.streamSource === "witanime") {
      if (!ep.witanimeEpisodeId) {
      ...
      const live = await witanimeEpisodeStream(ep.witanimeEpisodeId);
      rawUrl = live.url;
      type = live.type; // 'mp4' or 'embed'
      subtitles = live.subtitles?.length ? live.subtitles : subtitles;
      source = "witanime";
    } else if (ep.streamSource === "consumet") {
      ...
      const live = await consumetEpisodeStream(
        ep.consumetProvider,
        ep.consumetEpisodeId,
      );
      rawUrl = live.url;
      ...
      if (!ep.streamUrl?.trim()) {
        return res
          .status(400)
          .json({ message: "No stream URL configured for this episode" });
      }
      rawUrl = ep.streamUrl;
    ...
    const isIframeOrEmbed =
      type === "embed" || type === "iframe" || type === "external";
    ...
    if (!isIframeOrEmbed) {
      const resolvedType =
        type === "mp4" && !/\.m3u8/i.test(rawUrl) ? "mp4" : "hls";
      type = resolvedType;
      const referer =
        ep.streamSource === "consumet"
          ? refererForProvider(ep.consumetProvider)
          : "";
      playbackUrl = buildProxiedPlaybackUrl(
        rawUrl,
        ep._id.toString(),
        resolvedType,
        referer,
