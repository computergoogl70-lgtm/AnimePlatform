...
const episodeSchema = new mongoose.Schema(
  {
    ...
    witanimeEpisodeId: { type: String, default: "" },
...
episodeSchema.pre("validate", function validateStream() {
  ...
  if (this.streamSource === "witanime") {
    if (!this.witanimeEpisodeId) {
      this.invalidate("witanimeEpisodeId", "Witanime episode id is required");
    }
    return;
  }
  ...
  if (this.streamSource === "consumet") {
    if (!this.consumetProvider || !this.consumetEpisodeId) {
      this.invalidate(
        "consumetEpisodeId",
        "Consumet provider and episode id are required",
      );
    }
    return;
  }
